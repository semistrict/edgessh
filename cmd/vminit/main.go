...
func main() {
	...
	name := paramFromCmdline("edgessh_name")
	if name == "" {
		name = "edgessh-vm"
	}
	configureHostname(name)
...
func configureHostname(name string) {
	setHostname(name)
	if err := os.WriteFile("/etc/hostname", []byte(name+"\n"), 0o644); err != nil {
		fmt.Printf("failed to write /etc/hostname: %v\n", err)
	}
	...
	if err := os.WriteFile("/etc/hosts", []byte(hosts), 0o644); err != nil {
		fmt.Printf("failed to write /etc/hosts: %v\n", err)
	}
}
